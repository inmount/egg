...
namespace egg.Lark.Imports {
    ...
    internal class Hash {
        ...
        internal static void Reg(ScriptEngine engine) {
            ...
            hash["md5"] = new MemeryUnits.NativeFunction(engine.MemeryPool, (ScriptMemeryPool pool, ScriptEngine.FunctionArgs args) => {
            ...
            hash["getFileMD5"] = new MemeryUnits.NativeFunction(engine.MemeryPool, (ScriptMemeryPool pool, ScriptEngine.FunctionArgs args) => {
            ...
            hash["sha1"] = new MemeryUnits.NativeFunction(engine.MemeryPool, (ScriptMemeryPool pool, ScriptEngine.FunctionArgs args) => {
                string fnName = "hash.sha1";
                var content = args["content"];
                if (content.UnitType != MemeryUnits.UnitTypes.String) throw new Exception($"{fnName}函数的参数'content'不支持类型'{content.UnitType.ToString()}'");
                return engine.MemeryPool.CreateString(content.ToString().GetSha1()).MemeryUnit;
            }, egg.Strings.Create("content"));
            hash["sha256"] = new MemeryUnits.NativeFunction(engine.MemeryPool, (ScriptMemeryPool pool, ScriptEngine.FunctionArgs args) => {
                string fnName = "hash.sha256";
                var content = args["content"];
                if (content.UnitType != MemeryUnits.UnitTypes.String) throw new Exception($"{fnName}函数的参数'content'不支持类型'{content.UnitType.ToString()}'");
                return engine.MemeryPool.CreateString(content.ToString().GetSha256()).MemeryUnit;
            }, egg.Strings.Create("content"));
            hash["sha512"] = new MemeryUnits.NativeFunction(engine.MemeryPool, (ScriptMemeryPool pool, ScriptEngine.FunctionArgs args) => {
