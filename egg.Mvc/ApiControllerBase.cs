...
namespace egg.Mvc {
    ...
    public abstract class ApiControllerBase : Controller {
        ...
        private ApiControllerFieldSetting GetConfig(string name) {
            ...
            if (eggs.Object.IsNull(field)) field = FindConfig("*");
            return field;
        }
        ...
        protected void SetConfig(string name) {
            ApiControllerFieldSetting field = FindConfig(name);
            if (eggs.Object.IsNull(field)) AddConfig(name);
        }
        ...
        protected string Initialize() {
            ...
            if (_is_request_json) {
                // 获取提交的数据
                string content = eggs.Mvc.GetRequestData(base.Request);
                ...
                foreach (var cfg in this.Configs) {
                    if (cfg.IsMust) {
                        if (!JRequest.Form.Keys.Contains(cfg.Name)) {
                ...
                foreach (var key in JRequest.Form.Keys) {
                    var field = GetConfig(key);
                    if (!eggs.Object.IsNull(field)) {
                        if (field.Enabled) {
                            switch (field.DataType) {
                                case ApiControllerFieldSetting.DataTypes.Decimal: this.Form[key] = "" + (double)JRequest.Form[key]; break;
                                case ApiControllerFieldSetting.DataTypes.Long: this.Form[key] = "" + (long)JRequest.Form[key]; break;
                                case ApiControllerFieldSetting.DataTypes.Integer: this.Form[key] = "" + (int)JRequest.Form[key]; break;
        ...
        protected string Success(string msg = null) {
            JResponse.Result = 1;
            if (!msg.IsEmpty()) JResponse.Message = msg;
            // 重载
            this.OnRender();
            return JResponse.ToJsonString();
        }
        ...
        protected string Success(db.Rows rows, string msg = null) {
            JResponse.Result = 1;
            if (!msg.IsEmpty()) JResponse.Message = msg;
            JResponse.Datas = rows.ToJsonList();
            // 重载
            this.OnRender();
            return JResponse.ToJsonString();
        }
        ...
        protected string Error(int code = 0, string msg = null, string info = null) {
            JResponse.Result = -1;
            if (!msg.IsEmpty()) JResponse.Message = msg;
            if (!info.IsEmpty()) JResponse.ErrorInfo = info;
